...
from tensorflow_projection_qm.metrics import jaccard, continuity, trustworthiness, neighborhood_hit
...
def calculate_all_metrics_per_point(X, X_proj, k, y) -> dict[str, T.Tensor]:
    ms = tfpqm_metrics.metric.MetricSet(
        [
            trustworthiness.Trustworthiness(k=k).with_local(),
            continuity.Continuity(k=k).with_local(),
            neighborhood_hit.NeighborhoodHit(k=k).with_local(),
            jaccard.Jaccard(k=k).with_local(),
        ]
    )
    metric_vals: dict[str, tuple[T.Tensor, T.Tensor]] = ms.measure_from_dict(
        {"X": X, "X_2d": X_proj, "y": y}
    )
    return {k: v[1].numpy() for k, v in metric_vals.items()}
...
@hydra.main(version_base=None, config_name="exp2", config_path="conf")
def main(cfg: DictConfig):
    ...
    metric_learner = neural_qmetrics.models.metric_nnp.MetricLearner(X.shape[1], 2, out_dim=4)
    metric_learner.init_params()
    log.info(f"Projecting or reading previous {cfg.projection} projection")
    X_proj = read_or_generate_projection(X_train, cfg.dataset, cfg.projection)
    ...
    metric_names = ["trustworthiness", "continuity", "neighborhood_hit", "jaccard"]
    log.info(f"Calculating all metrics to be optimized ({metric_names}), using k={cfg.k}")
    metric = calculate_all_metrics_per_point(X_train, X_proj, k=cfg.k, y=y_train)
    # make into a dataset of targets
    metric = np.stack([metric[m] for m in metric_names], axis=1)
    log.info("Done")
